...
import { getAllMeasurementUnits, createMeasurementUnit, updateMeasurementUnit, deleteMeasurementUnit } from '@/controller/inventory-settings';
...
export async function GET(request: NextRequest) {
  ...
  const items = await getAllMeasurementUnits(user.currentTenantId, search);
...
export async function DELETE(request: NextRequest) {
  ...
  if (!id) return NextResponse.json({ data: null, error: 'ID is required' }, { status: 400 });
  const deleted = await deleteMeasurementUnit(user.currentTenantId, user.id, id);
