...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    // "OWN" archive: verify the user created this fuel transaction
    ...
    if (perms.archive === 'OWN') {
      ...
      if (existing.createdBy?.toString() !== user.id) {
        return NextResponse.json({ data: null, error: 'You can only archive fuel entries you created' }, { status: 403 });
      }
    }
