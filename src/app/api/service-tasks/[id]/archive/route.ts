/**
 * PATCH /api/service-tasks/:id/archive -- Archive or unarchive a service task
 */
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    // "OWN" archive: verify the user created this service task
    ...
    if (perms.archive === 'OWN') {
      ...
      if (existing.createdBy?.toString() !== user.id) {
        return NextResponse.json({ data: null, error: 'You can only archive service tasks you created' }, { status: 403 });
      }
    }
