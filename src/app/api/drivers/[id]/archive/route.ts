/**
 * PATCH /api/drivers/:id/archive -- Archive or unarchive a driver
 */
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    // "OWN" archive: verify the user created this driver
    ...
    if (perms.archive === 'OWN') {
      ...
      if (existing.createdBy?.toString() !== user.id) {
        return NextResponse.json({ data: null, error: 'You can only archive drivers you created' }, { status: 403 });
      }
    }
