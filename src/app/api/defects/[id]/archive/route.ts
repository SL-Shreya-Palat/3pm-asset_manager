/**
 * PATCH /api/defects/:id/archive -- Archive or unarchive a defect
 */
import { NextRequest, NextResponse } from 'next/server';
import { ObjectId } from 'mongodb';
import { getAuthenticatedUser } from '@/lib/auth-helper';
import { getDefectsCollection } from '@/lib/mongodb';
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    // "OWN" archive: verify the user created this defect
    ...
    if (perms.archive === 'OWN') {
      ...
      if (existing.createdBy?.toString() !== user.id) {
        return NextResponse.json({ data: null, error: 'You can only archive defects you created' }, { status: 403 });
      }
    }
