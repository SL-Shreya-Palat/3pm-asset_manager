/**
 * PATCH /api/faults/:id/archive -- Archive or unarchive a fault
 */
import { NextRequest, NextResponse } from 'next/server';
import { ObjectId } from 'mongodb';
import { getAuthenticatedUser } from '@/lib/auth-helper';
import { getFaultsCollection } from '@/lib/mongodb';
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    // "OWN" archive: verify the user created this fault
    ...
    if (perms.archive === 'OWN') {
      ...
      if (existing.createdBy?.toString() !== user.id) {
        return NextResponse.json({ data: null, error: 'You can only archive faults you created' }, { status: 403 });
      }
    }
