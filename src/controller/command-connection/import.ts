 ...
 *
 * Keeps driver RECORDS mirrored from Command (profile fields via the shared
 * mapper). It deliberately does NOT create users/tenantMembers or send
 * invitation emails — this runs on a 30s throttle behind every drivers-list
 * read, and unsolicited login invites blasting out on page views is not
 * acceptable. Granting login access is an explicit action: the
 * "Import from Command" dialogs (Users / Drivers pages) create the membership
 * and send the invite.
 */
async function importDrivers(
  ...
  if (ops.length > 0) {
    const result = await drivers.bulkWrite(ops, { ordered: false });
    counts.created = result.upsertedCount;
    counts.updated = result.matchedCount;
  }
  return counts;
}
