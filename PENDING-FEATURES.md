...
> **Status (2026-07-03): ASSET scope + expiry REMINDERS SHIPPED.** Built: `documents` collection wiring, `controller/documents/{index,types,utils}` (+ `listExpiring`), `/api/documents` + `/api/documents/[id]`, `constants/documents.ts` (NZ-friendly types — Rego/WOF/COF/RUC/insurance/permit), the `AssetComplianceTab` (colour-coded Valid/Expiring/Expired status, add/edit, file upload, **one-tap Renew** = a PUT of new expiry + file), mounted on the asset detail page. Status is derived server-side; expiry-day counts as "expiring soon". **Reminder scan (step 8) DONE** — `scanComplianceExpiry` in `controller/notifications/scan.ts` runs from the existing `/api/cron/notifications` cron, notifies managers + the assigned driver (deduped via `notifyTenantManagersOnce`, one alert per doc per window), with `document_expiring`/`document_expired` notification types. **Assets-list compliance column + SERVER-SIDE filter DONE** — `getAllAssets` enriches each visible asset with a worst-case `complianceStatus` (expired>expiring_soon>valid>none) via one bounded query over `documents` (`computeComplianceStatusMap`, paginate-before-enrich safe), rendered as a colour-coded "Compliance" column in `asset-table.tsx`. The **compliance filter** is implemented SERVER-side (`complianceAssetIdClause` → `$in`/`$nin` `_id` set resolved fleet-wide BEFORE pagination, so it's accurate across all pages — NOT the client-only path the other toolbar filters use); the table's `<Select>` passes `?complianceStatus=` to the API and refetches from page 1. **Still pending:** the **Driver** and **Company** scopes (schema + validators already generic — need scope tabs), and the dashboard/Exception Report surfacing (dashboard tiles are still unwired placeholders — wire them together, not one-off).
